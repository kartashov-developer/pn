...
. `MIME type:` |{json}application/json|.
  ...
  # null |null|
    JSON syntax .
    . JSON is nested in `{object}`. JSON-object - is an unordered set of 
      "key(ключ)": "value(значение)".
    ! Key its the name of the parameter that we pass to the web-server or 
      take from it. `The key must always be in double quotes`.
