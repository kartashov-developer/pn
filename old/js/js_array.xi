...
. Массивы располагаются строго по порядку. Ключ всегда `фиксирован`. Порядок имеет важную роль и он `не меняется`.
...
| console.log(bla2); // [ 10, 2, 3 ]
  ...
  . Работает только с примитивами. Если есть вложенные структуры, то новый массив будет хранить только `ссылку` на них(изменяют их - изменяется и animals2).
  |{js}
