...
Использование git .
  ...
  |{zsh}git init| - появится скрытая папка |.git|, в которой содержится вся история изменений.
  |{zsh}git status| - проверяем состояние репозитория.
  |{zsh}git add .| - проиндексировать все файлы репозитория.
  |{zsh}git add css/button.css css/main.css| - проиндексировать указанные 2 файла
  |{zsh}git commit -m "description"| - зафиксировать изменения(сохраниться)
  |{zsh}git diff| - посмотреть, что изменилось(красным - удаленное, зеленым - добавленное)
...
Восстановить файл .
  ...
  |{zsh}git checkout index.js| - верне файл в то состояние, в котором он был в последнем коммите
