...
Data type[] .
  Primitive data type[] .
  . No primitive data type, everything is an object.
  Composite data type[] .
  . Via [lua table]s.
  Anonymous data type[] .
  . Via [lua table]s.
...
. |function|, |end| pair forms an expression:
  ...
  Anonymous function .
  . As in [js] and [scheme] all functions are anonymous. A "named function" 
    is a variable holding a reference to a function object.
...
xi test @
. Basic loop is 100 times faster compared to [python]/[csharp]/[php]
[#1] [#3]. [nuxt]
...
. Todo .
  ...
  {index(type)}
  {index variable(type is awful) settings mine}
  {index index}
  {index}
  {i}
  ?text text?
...
. Region
  ...
  sdf`sdf`
	...
	. |{x}|: Non-text item and |:|.
	|{py}a = 1| inline code with spaces, one per line, no '|'.
	|{py}a=1| inline code without spaces, |{py}a=1|2|.
	. |{py}a = 1|: inline code after paragraph.
	. |{lng:js}
	| [notlink]
	. | [notlink]
	[jump test]
    ...
    h3 .
      h4 .
        h5 .
          h6 .
h1 [link] .
. Some text about `important` parts of using [UI] components.
. Essential is the object for |useForward|.
. No exceptions with such primitive types, like all. Use another brackets for 
  this extension. Hope we helped you.
. Create an |array|: 
  |{lng:js}
  | const valueNumberOne = 'string about MINE' -- settings bitch
  h2 [link] .
  . Some text about `important` parts of using [UI] components.
  . Essential is the object for |useForward|.
  . No exceptions with such primitive types, like all. Use another brackets 
    for this extension. Hope we helped you.
  . Create an |array|: 
    |{lng:js}
    | const valueNumberOne = 'string about MINE' -- settings bitch
    h3 [link] .
    ...
    . Create an |array|: 
      ...
      . Some text about `important` parts of using [UI] components.
      . Essential is the object for |useForward|.
      . No exceptions with such primitive types, like all. Use another 
        brackets for this extension. Hope we helped you.
      . Create an |array|: 
        |{lng:js}
        | const valueNumberOne = 'string about MINE' -- settings bitch
        h5 [link] .
...
Strange symptom! We don't have any specialized ?bar?.
|{js}
| const firstName = 'Vyacheslav'
What do you think about this |func|?
h1 .
  h2 .
    h3 .
      h4 .
