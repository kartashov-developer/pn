...
Что такое Element? .
. Элемент - это кусочек HTML в DOM-дереве. Каждый HTML-тег превращается в 
  элемент DOM.
. Из DOM можно получить элемент и изменить его. Браузер заметит изменения и 
  отобразит их на странице.
Свойства Element .
. HTML-элементы содержат свойства, которые можно разделить на группы:
  Свойства, связанные с HTML-атрибутами: .
    ...
    | const button = document.querySelector("button");
      ...
      . Принимает 2 параметра: 
        {n(что нужно заменить) 1st}
        {o(на что нужно заменить) 2nd},
        ...
        | const result = button.classList.replace("hidden", "visible");  
                                                     ⬆ меняем hidden на visible
      toggle() .
      contains() .
      |{js}  
      | button.classList.contains("active"); 
      // содержимое классов(boolean: true|false)
    style .
    . Добавить стили:
  ...
  Информация о содержимом: .
    innerHTML .
    . Возвращает HTML-код всего, что вложено в текущий элемент. При записи в 
      это свойство, предыдущее содержимое будет затерто. Страница отобразит 
      новое содержимое. Приводит к `перерисовке`.
    . Принимает {string}.
