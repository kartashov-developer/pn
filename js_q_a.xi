[https://disk.yandex.ru/d/IM3zCu5aicZq1Q]
...
HTML .
  ...
  . HTML(HyperText Markup Language) - язык гипертекстовой разметки. Является 
    скелетом любой страницы в интернете.
...
JS .
  ...
  . string, number, boolean, null, undefined, BigInt, Symbol, object. |8 типов|
  Разница между == и === (нестрогое/строгое равенство)? .
  . [js_comparison].
  . Возвращают |boolean|.
  . |==| приводит оба значения к общему типу.
  . |===| не приводт к общему типу, а сравнивает только значения.
    ! Если типы не равны - выдаст {false}.
  Что такое Strict mode в JavaScript? .
  . [js_strict]. Сообщает, что работа ведется в современном режиме, где исправлены 
    старые возможности. Помогает избежать различных ошибок.
    # Запрещает удалять переменные и ф-ии с помощью {delete}.
    # Запрещает дублировать аргументы ф-ии.
    # Запрещает использовать зарезервированные слова.
    # Запрещает создавать переменные без ключевых слов.
  Расскажи про Function Declaration? .
  . [js_functions].
  . Объявление происходит с помощью ключевого слова |function|.
  . Ф-ия, определенная подобным образом, может быть вызвана в любом месте кода, 
    даже до самого объявления ф-ии. Создается до выполнения скрипта.
  . Обычно используются для создания именованных ф-ий, которые могут быть 
    вызваны из любой части кода.
  Расскажи про Function Expression? .
  . [js_functions]. 
  . Объявление происходит также с помощью `ключевого` слова |function|, но 
    затем ф-ия присваивается переменной.
  . Ф-ия, определенная подобным образом, не поднимается, поэтому 
    должна быть определена до вызова.
  . Часто используются для создания анонимных ф-ий или для присваивания ф-ий 
    переменным.
  Расскажи про Function Expression? .
  . [js_functions]. 
  ...
  # |во времени их создания|
    Declaration(Объявление) ф-ий и переменных поднимаются вверх, в начало 
    области видимости, поэтому их можно использовать в коде до самого 
    объявления.
  # |доступности в коде|
    Expression(Выражения) ф-ий и переменных создаются в момент выполнения кода, 
    поэтому их можно использовать только после их определения.
  Расскажи про глобальные и локальные переменные? .
  . [js_functions]. 
  ! Переменные, объявленные внутри функции, видны только внутри этой функции.
  . У функций есть доступ к глобальным переменным(созданные вне функции). Она 
    даже может изменять их значение.
  ...
  Типы таймеров в JavaScript? Что они принимают? Чему равна задержка по .
  умолчанию? Как очистить заданные таймеры? .
  [js_settimeout].
  . setTimeout. {function}, {delay}, {arguments}. {delay}=^0^. |clearTimeout|.
  . setInterval. {function}, {delay}, {arguments}. {delay}=^0^. |clearInterval|.
  Что такое поднятие (Hoisting)? .
  . Hoisting - механизм, при котором объявления переменных и ф-ий перемещаются в 
    начало их области видимости перед выполнением кода.
    То есть использовать переменные и вызывать функции до их объявления.
 ...
 "quote",
  ...
  В чем отличие между isNaN() и Number.isNaN()? .
  ...
  . Использовать |Number.isNaN()| или |isNaN()|, чтобы определить равно ли 
    значение {NaN}. Можно создать свою ф-ию.
