...
Ментальные модели .
. Для каждой знакомой фундаментальной концепции программирования(например, переменная или операция с ней) существует набор глубоко укоренившихся аналогий, которые вы с ней связываете. Некоторые из них пришли из реального мира. Другие могут быть перепрофилированы из других областей, котоыре вы изучили первыми(математика). Эти аналогии могут пересекаться и даже противоречить друг другу, но они все равно помогают понять, что происходит в коде.
  К примеру, многие люди впервые узнали о переменных, как о 'коробках' - контейнерах, в которых хранятся данные. Даже, если вы больше не представляете ящики, когда видите переменную, они все равно могут вести себя в вашем воображении 'ящиками'.
  Эти приблизительные представления о том, как что-то работает в голове, известны, как `ментальные модели`. Нужно постораться замечать и анализировать свои ментальные помеди. Вероятно, они представляют собой комбинацтю визуальных, пространственных и механических умственных сокращений. Эта интуиция(например, 'квадратность' переменных) влияет на то, как мы читаем код всю нашу жизнь.
  К сожалению, иногда ментальные модели могут быть ошибочны. Может быть, учебник, который мы прочитали ранее, пожертвовал точностью, чтобы упростить объяснение концепции. Возможно, когда мы начинали изучать JavaScript, мы неправильно 'перенесли' ожидаемое поведение из языка, который изучали ранее. Возможно, мы вывели ментальную модель из какого-то фрагмента кода, но никогда не проверяли ее правильность.
  Хорошая ментальная модель поможет получить уверенность в своем коде, понять и исправить чужой код.
Вселенная JavaScript .
. В начале были `значения`. Что же такое значение? Сложно сказать. Что такое точка в геометрии? Значение - фундаментальное понятие в JavaScript, поэтому мы не можем определить его с помощью других терминов. Вместо этого мы определим его на примерах. Числа и строки являются значениями. Объекты и функции - тоже. Есть также много вещей, которые не являются значениями, например фрагменты нашего кода - например, наши операторы if(if statements), циклы(loops) и объявления переменных(variable declarations).
. Когда мы начинаем строить нашу ментальную модель, одно из первых распространенных заблуждений, которые нам нужно прояснить, состоит в том, что значения - это наш код.
Вместого этого нам нужно думать о них отдельно - наш код взаимодействует со значениями, но значения существуют в совершенно отдельном пространстве.
  Значения .
  . Вообще-то существует два вида значений. |Примитивные| и |Объекты и функции|.
    ...
    . Примитивные значения подобны звездам - холодным и далеким, но они всегда рядом, когда нужны. Даже с поверхности моей маленькой планеты, я могу найти и указать на них. Среди прочего это могут быть числа и строки. У всех примитивных значений есть кое-что общее: они являются постоянной частью нашей вселенной JavaScript, так как их нельзя создать, изменить или разрушить. Только указать.
    Объект и функции .
    Object & Function (Mutable Values) .
    . Объекты и функции также являются значениями, но в отличие от примитивных значений, я могу манипулировать ими из своего кода. Если примитивные значений подобны звездам, то объекты и функции подобны астероидам, которые летают вокруг моей планеты - моего кода. Они не являются частью кода, но они достаточно близки, чтобы ими можно было манипулировать.
    | console.log({}); -- {}
    | console.log([]); -- []
    | console.log(x => x * 2); -- [Function (anonymous)]
  . Поначалу все значения могут выглядеть одинаково - просто точки в небе. Но мы здесь, чтобы изучить все различные вещи, плавающие над нами в нашей вселенной JavaScript, поэтому понадобится способ классификации.
    . Мы можем разбить значения на типы - значения одного типа ведут себя одинаково. Всего их 9:
    `Примитивные`
    # {undefined(случайно неопределенный) используется для непреднамеренно пропущенных значений},
    # {null(специальное неопределенный) используется для намеренно пропущенных значений},
    ...
    ! |Других типов нет|
      В JavaScript нет фундаментальных типов значений, кроме тех, которые мы перечислили. `Остальное все объекты`! Даже массивы, даты и регулярные выражения являются объектами:
