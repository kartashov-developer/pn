...
Объекты .
. В JavaScript сущетвует 8 типов данных. 7 из них являются `примитивными`. 
  Единственный не примитивный из них - объект.
. Объекты используются для хранения коллекций различных значений и более 
  сложных сущностей.
...
for in - перебор объекта .
  ?let? {объект (в каком объекте мы все будем перебирать?)} 
  ?in? {имя свойства (как каждое свойство назвать)}
  |{js}  
