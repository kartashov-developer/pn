...
for .
  ...
  |тело| - выполняется снова и снова, пока условие не станет |true|.
...
Вложенные циклы .
  ...
  | *****
    ...
    | let result = "";
    | const length = 7;
    | for (let i = 1; i < length; i++) { // Icycle
    |   for (let j = 0; j < i; j++) { // Jcycle
    |     result += "*";
    |   }
    |   result += "\n";
    |   i;
    ...
    . {result += '\n';} - перенос строки, который будет происходить каждый раз 
      при выходе из <Jcycle>.
    . ^(j < i)^ - каждый раз, когда строка переходит, мы должны увеличивать 
      количество звездочек, которые туда попадут.
    ! Переменная result обновляется после каждого нового изменения в условии 
      |Icycle|.
    ! |i| стало 1? => |{js}result = ''|.
    ! |i| стало 2? => |{js}result = ''|.
    # ?Итерация 1? Входим в цикл |Icycle|. |i| = 1. 
      Входим в цикл <Jcycle>. <j> = 0.  
      <j(0)> < |i(1)| = true.
      | result = '' + '*' => '*'.
      . Что дальше? Продолжаем итерации в цикле <Jcycle>, но по условию <j> уже 
        равно 1 и <j(1)> < |i(1)|.
        Не подходит => выходим из цикла.
        Входим в тело цикла |Icycle|.
      | result = '*' + '\n'(перенос строки) => * с переносом на следующую строчку.
    # ?Итерация 2? Цикл прогнан, теперь в |Icycle| увеличиваем |i|. |i| = 2. 
      Входим в цикл <Jcycle>. <j> = |0|. 
      <j(0)> < |i(2)| = true.
      | result = '' + '*' => '*'.
      . Что дальше? Продолжаем итерации в цикле <Jcycle> и по условию видим, что
        ...
        | result = '*' + '*' => '**'.
      . Что дальше? Продолжаем итерации в цикле <Jcycle>, но по условию <j> 
        увеличилось на 1 и стало равно 2. <j(2)> < |i(2)|.
        Не подходит => выходим из цикла.
        Входим в тело цикла |Icycle|.
        | result = '**' + '\n'(перенос строки) // ** с переносом на следующую строчку.
  Пример 3. Задача 2 .
  ...
  | ***********
    ...
    | const lines = 5;
    | let mainString = "";
    | for (let i = 0; i < 6; i++) { // Icycle
    |   for (let j = 0; j < lines - i; j++) { // Jcycle1
    |     mainString += "_";
    |   }
    |   for (let j = 0; j < 2 * i + 1; j++) { // Jcycle2
    |     mainString += "*";
    |   }
    |   mainString += "\n";
    | }
    ...
    # ?Итерация 1? Входим в |Icycle|. |i| = 0; 0 < 6.
      ...
      . В теле цикла видим: {mainString = '' + '_' = '_'}. 
        Пытаемся выйти из <Jcycle1>, обновляя <j> = 1; 1 < 5 - |i(0)|.Повторяем.
      . {mainString = '_' + '_' = '__'}. Пытаемся выйти из <Jcycle1>, обновляя 
        <j> = 2; 2 < 5 - |i(0)|. Повторяем.
      . {mainString = '__' + '_' = '___'}. Пытаемся выйти из <Jcycle1>, обновляя 
        <j> = 3; 3 < 5 - |i(0)|. Повторяем.
      . {mainString = '___' + '_' = '____'}. Пытаемся выйти из <Jcycle1>, обновляя 
        <j> = 4; 4 < 5 - |i(0)|. Повторяем.
      . {mainString = '____' + '_' = '_____'}. Пытаемся выйти из <Jcycle1>, 
        обновляя <j> = 5; 5 < 5 - |i(0)|. Выходим.
      Входим в ?Jcycle2?. <j> = 0; 0 < 2 * 0 + 1; 0 < 1.
      {mainString = '_____' + '*' = '_____*'}
    # ?Итерация 2? Входим в |Icycle|. |i| = 1; 1 < 6.
      Входим в <Jcycle1>. <j> = 0; 0 < 5 - |i(1)| = 0 < 4.
      . В теле цикла видим: {mainString = '' + '_' = '_'}. Пытаемся выйти из 
        <Jcycle1>, обновляя <j> = 1; 1 < 5 - |i(1)| = 1 < 4. Повторяем.
      . {mainString = '_' + '_' = '__'}. Пытаемся выйти из <Jcycle1>, обновляя 
        <j> = 2; 2 < 5 - |i(1)| = 1 < 4. Повторяем.
      . {mainString = '__' + '_' = '___'}. Пытаемся выйти из <Jcycle1>, обновляя 
        <j> = 3; 3 < 5 - |i(1)| = 1 < 4. Повторяем.
